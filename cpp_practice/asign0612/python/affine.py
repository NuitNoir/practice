  ...
  def TransformE(self, src, M, flag):
      ...
      for y in range(rows):
        for x in range(cols):
          ...
          p = M.dot(p1)
          if (flag == 0):
              val = self.NN(src, p[0], p[1])
          elif (flag == 1):
              val = self.Bilinear(src, p[0], p[1]);
          else:

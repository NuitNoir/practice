	...
	public:
		...
		void draw_skel_perspective(Mat &img, const SKEL &skel, unsigned char gray) {
		  ...
		  //skel.print_polylines();
	        ...
	        // cout << z << ' '<< p1z << ' ' << p2z << endl;
	        // if (p1z > z || p2z < z) {
	        // 	cout << _location(2,3)<<' '<<p1z<<' '<<p2z << endl; 
	        // }
	        // if (p1z < z && p2z < z) {
 ...
 */
  ...
  Point2f vec1(p0[0]-p1[0], p0[1]-p1[1]);
  Point2f vec2(p2[0]-p1[0], p2[1]-p1[1]);

  // double inner_product = vec1.ddot(vec2);
  double inner_product = vec1.x*vec2.x + vec1.y*vec2.y;

  // cout << inner_product << vec1 << vec2 <<  endl;

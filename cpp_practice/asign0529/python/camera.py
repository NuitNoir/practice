...
class Camera:
  ...
  def draw_skes_perspective(self, img, skels, gray):
    ...
    for i in range(0, len(skels)):
      ...
      skels[i].transform(R)
      self.draw_skel_perspective(img, skels[i], gray)

  def draw_skel_perspective(self, img, skel, gray):
    proj_vecs = np.ndarray((1, 3))
    ...
    for line in skel.lines:
      for j in range(1, int(line[0])):
        if (line[0] >=3):
          ...
          hide_flg = False
        pt1 = np.zeros((0, 2))
        pt1 = proj_vecs[line[j]]
        pt2 = proj_vecs[line[j+1]]
        cv2.line(img, (int(pt1[0]), int(pt1[1])), (int(pt2[0]), int(pt2[1])), gray)

...
int main() {
	cv::Matx33d M;
	cv::Mat src = cv::imread("../cat/cat1.pgm", 0);
...
cv::Mat TransformL(cv::Mat &src, const cv::Matx33d &M) {
	...
	for (int y=0; y<dst.rows; y++) {
		for (int x=0; x<dst.cols; x++) {
			...
			if (p[1] < dst.rows && p[0]<dst.cols && y>=0 && x>=0) dst.at<unsigned char>(p[1], p[0]) = val;
		}
	}
	return dst;
}
	...
	for (int y=0; y<dst.rows; y++) {
		for (int x=0; x<dst.cols; x++) {
			...
			if (flag == 0){
				...
				cout << "flag must be 0:Nearest Neighbour, 1:Bilinear Interpolation" << endl;
				break;
			}
			if (y < dst.rows && x<dst.cols && y>=0 && x>=0) dst.at<unsigned char>(p1[1], p1[0]) = val;
		}
	}
	return dst;
}
...
unsigned char Bilinear(Mat &src, double x, double y) {
	...
	val = (1-diff_y)*((1-diff_x)*src.at(ix, iy) + diff_x*src.at(ix+1, iy))
			+ diff_y*((1-diff_x)*src.at(ix, iy+1) + diff_x*src.at(ix+1, iy+1));

	return val;
}

cv::Matx33d Rotation(double theta) {
	Matx33d R(cos(theta), -sin(theta),0,
